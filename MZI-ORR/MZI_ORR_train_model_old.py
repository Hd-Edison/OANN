...
Model = Sequential()
Model.add(Dense(80, input_dim=100))
Model.add((ReLU()))
...
es = EarlyStopping(monitor='val_loss', mode='auto', verbose=1, patience=1000)
...
history = Model.fit(x_train, y_train, epochs=5000,
                    batch_size=100, validation_data=(x_val, y_val), callbacks=[es])
