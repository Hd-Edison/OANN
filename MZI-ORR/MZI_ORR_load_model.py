...
if __name__ == '__main__':
    ...
    ## use calculation
    if use_calculation:
        # true_function = RELU(0.45, z_squared).numpy()  # Your new data as a NumPy array
    ...
    #                                                           z_squared=z_squared)))
    predicted_relu = H ** 2 * z_squared

    ### draw comparation curve
    plot_prediction_curve(z_squared, true_function, predicted_relu, "RELU")
    print(predictions)
    print("rmse = ", rmse(y_true=true_function, y_pred=predicted_relu).numpy())
    print("rmse in dB = %f dB"% (10 * np.log10(rmse(y_true=true_function, y_pred=predicted_relu).numpy())))
