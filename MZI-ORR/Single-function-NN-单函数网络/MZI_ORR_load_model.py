...
def H(kr, phi, a, b, k1, k2, z_squared):
    ...
    return abs(oann.transfer_function_MZI_ORR(HR=oann.transfer_function_ORR(k=kr, gamma=0.8, neff=2.384556,
                                                                            phi=oann.calculate_phi(a * 10 * 2 * np.pi, b * 10,
                                                                                                   z_squared=z_squared),
                                                                            L=5e-6 * 2 * np.pi),
...
def H_new(kr, phi, a, b, k1, k2, z_squared):
    ...
    H = np.empty(200)
    for i in range(200):
        m = abs(oann.transfer_function_MZI_ORR(HR=oann.transfer_function_ORR(k=kr[i, 0], gamma=0.8, neff=2.384556,
...
if __name__ == '__main__':
    ### load model
    model = load_file("OANN_model.json", "OANN_model.weights.h5")
